const RatingAndReview = require("../models/RatingAndReview");
const Course = require("../models/ServiceCenter");
const { mongo, default: mongoose } = require("mongoose");

exports.createRating = async (req, res) => {
    try{
        const userId = req.user.id;
        const {rating, review, serviceCenterId} = req.body;

        const alreadyReviewed = await RatingAndReview.findOne({
                                                user:userId,
                                                serviceCenter:serviceCenterId,
                                            });
        if(alreadyReviewed) {
                    return res.status(403).json({
                        success:false,
                        message:'Already reviewd by user',
                    });
                }
        //create raring and review
        const ratingReview = await RatingAndReview.create({
                                        rating, review, 
                                        serviceCenter:serviceCenterId,
                                        user:userId,
                                    });
       
        //update servicecenter
        await Course.findByIdAndUpdate({_id:serviceCenterId},
                                    {
                                        $push: {
                                            ratingAndReviews: ratingReview._id,
                                        }
                                    },
                                    {new: true});
         return res.status(200).json({
            success:true,
            message:"Rating and Review created Successfully",
            ratingReview,
        })
    }
    catch(error) {
        console.log(error);
        return res.status(500).json({
            success:false,
            message:error.message,
        })
    }
}

exports.getAverageRating = async (req, res) => {
    try {
            const serviceCenterId = req.body.serviceCenterId;

            //calculate average ratings
            const result = await RatingAndReview.aggregate([
                {
                    $match:{
                        course: new mongoose.Types.ObjectId(serviceCenterId),
                    },
                },
                {
                    $group:{
                        _id:null,
                        averageRating: { $avg: "$rating"},
                    }
                }
            ])

            //return rating
            if(result.length > 0) {

                return res.status(200).json({
                    success:true,
                    averageRating: result[0].averageRating,
                })

            }
            
            // if not rated yet
            return res.status(200).json({
                success:true,
                message:'Avg Rating is 0, no ratings given till now',
                averageRating:0,
            })
    }
    catch(error) {
        console.log(error);
        return res.status(500).json({
            success:false,
            message:error.message,
        })
    }
}

exports.getAllRating = async (req, res) => {
    try{
            const allReviews = await RatingAndReview.find({})
                                    .sort({rating: "desc"})
                                    .populate({
                                        path:"user",
                                        select:"username email image",
                                    })
                                    .populate({
                                        path:"serviceCenter",
                                        select: "name",
                                    })
                                    .exec();
            return res.status(200).json({
                success:true,
                message:"All reviews fetched successfully",
                data:allReviews,
            });
    }   
    catch(error) {
        console.log(error);
        return res.status(500).json({
            success:false,
            message:error.message,
        })
    } 
}